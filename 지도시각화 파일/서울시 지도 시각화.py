...
df = pd.read_csv('data_with_scores_FINAL.csv', encoding='utf-8-sig')
...
for score_col, color_theme, legend in map_configs:
    ...
    white_text_dongs = [
        '종로1·2·3·4가동', '종로5·6가동', '청운효자동', '상계3·4동', 
        '상계6·7동', '중계2·3동', '청량리동', '면목3·8동', 
        '우장산동', '금호2·3가동', '명일2동', '상일동', 
        '반포본동', '일원2동', '남현동', '항동'
    ]
    ...
    for feature in geo_data['features']:
        ...
        if dong_name in white_text_dongs:
            font_color = 'white'
        else:
            font_color = 'black'
        ...
        folium.Marker(
            ...
            icon=DivIcon(
                icon_size=(150,36),
                icon_anchor=(75,18),
                # 여기서 color에 방금 정한 {font_color}를 넣어줍니다.
                html=f'<div style="font-size: 5pt; color: {font_color}; text-align: center; font-weight: bold;">{dong_name}</div>',
            )
        ).add_to(m)
